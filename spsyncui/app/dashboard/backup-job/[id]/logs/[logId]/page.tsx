...
import {
  ...
  getPlaceholderBackupJobLogById,
  getPlaceholderBackupJobLogFiles,
} from "@/lib/backup-jobs/placeholder-data";

export default function DashboardBackupJobLogDetailPage() {
  const params = useParams<{ id: string; logId: string }>();
  const jobId = params.id ?? "";
  const logId = params.logId ?? "";

  const job = getPlaceholderBackupJobById(jobId);
  const log = getPlaceholderBackupJobLogById(logId);
  const files = getPlaceholderBackupJobLogFiles(logId);
  ...
  return (
    <main className="p-6">
      <div className="mb-6 flex items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold">Run log detail</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {job.sharepointSiteUrl} · {formatLogStatus(log.status)}
          </p>
        </div>
        <Button asChild variant="outline">
          <Link href={`/dashboard/backup-job/${job.id}`}>Back to job</Link>
        </Button>
      </div>
